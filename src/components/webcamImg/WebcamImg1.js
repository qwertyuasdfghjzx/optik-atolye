...
// DİKKAT: Camera utils'i sildik, manuel döngü kullanacağız.
import Info from "../../components/info/Info";

const WebcamImg = () => {
  const webcamRef = useRef(null);
  const canvasRef = useRef(null);
  const requestRef = useRef(null); // Döngü kontrolü için
  ...
  // --- KAMERA AYARLARI ---
  // Başlangıç: ARKA KAMERA ('environment')
  ...
  // Video Ayarları: Arka kamera için 'exact' kullanarak zorluyoruz
  ...
  useEffect(() => {
    ...
    faceMesh.onResults((results) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
        ...
        const drawCross = (x, y) => {
            canvasCtx.beginPath();
            canvasCtx.moveTo(x - 15, y); canvasCtx.lineTo(x + 15, y);
            canvasCtx.moveTo(x, y - 15); canvasCtx.lineTo(x, y + 15);
            canvasCtx.stroke();
        };
    ...
    // --- MANUEL DÖNGÜ (Camera Utils Yerine) ---
    // Bu yöntem kamerayı kilitlemez, webcam ne gösterirse onu işler.
    const runDetection = async () => {
      if (
        ...
        try {
          ...
          console.log("FaceMesh Error:", e);
        }
      }
      // Bir sonraki kare için tekrar çağır (Sonsuz döngü)
    ...
    return () => {
      // Temizlik: Döngüyü durdur, FaceMesh'i kapat
      if (requestRef.current) cancelAnimationFrame(requestRef.current);
      faceMesh.close();
    };
  }, [facingMode]); // Mod değişince her şey sıfırlanıp yeni kamera ile başlar
      ...
      <div className="container-app" style={fullScreenStyle}>
        ...
        <div className="container-display" style={{ display: "none", ...fullScreenStyle }}>
          <div style={{position: 'relative', flex: 1, width: '100%', height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#111'}}>
             ...
             {/* Key='facingMode' sayesinde mod değişince kamera sıfırlanır ve yeni lens açılır */}
             <Webcam 
                key={facingMode} 
                ref={webcamRef} 
                videoConstraints={videoConstraints} 
                audio={false} 
                mirrored={facingMode === "user"} // Sadece ön kamerada ayna
